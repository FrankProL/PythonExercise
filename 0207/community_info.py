...
class community_info(object):
    ...
    def do(self):
        ...
        cnxn = MySQLdb.connect(host=self.into_db[0], user=self.into_db[1], passwd=self.into_db[2], charset=self.into_db[3],db='stats_dev')
        cursor = cnxn.cursor()
        sql = "select name from tmp_community_info where  city = '{}' ".format(self.city)
        cursor.execute(sql)
        ...
        cnxn = MySQLdb.connect(host=self.into_db[0], user=self.into_db[1], passwd=self.into_db[2],
                               charset=self.into_db[3],db='stats_dev')  ###
        cursor = cnxn.cursor()
        ...
        for x in dict_data_list :  # 遍历字典列表
            if not x['name'].decode('utf-8') in url_database:  # 判断小区是否已经存在
                sql = "insert into tmp_community_info ({}) values ({})".format(
